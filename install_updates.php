...
/** Add the dedicated category SEO meta-description column and refresh public routes. */
function DOCUMENTS_updateSchema_1_2_0()
{
    ...
    if (isset($_DB_dbms) && $_DB_dbms !== 'mysql') {
        if (function_exists('COM_errorLog')) {
            COM_errorLog('Documents 1.2.0 schema upgrade requires the MySQL/MariaDB backend.');
        }
        return false;
    }
    ...
    $schemaReady = DB_numRows($result) > 0;
    ...
    if (!$schemaReady) {
        ...
        $schemaReady = DB_numRows($verify) > 0;
    }
    ...
    return DOCUMENTS_writeHtaccess(true);
}
