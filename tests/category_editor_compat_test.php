...
documents_category_editor_forbid($editor, 'COM_createHTMLDocument($content, array(', 'Category editor passes a temporary array to COM_createHTMLDocument().', $failures);
...
documents_category_editor_require($template, '<style>', 'Category editor does not provide scoped fallback styling when plugin CSS is absent from the page head.', $failures);
documents_category_editor_require($template, '.documents-category-form{', 'Scoped category editor styles are missing.', $failures);
documents_category_editor_require($template, '.documents-category-form .documents-form-grid{display:grid;', 'Two-column responsive field grid is missing.', $failures);
documents_category_editor_require($template, 'grid-template-columns:minmax(0,1fr) minmax(0,1fr)', 'Desktop category editor does not use a two-column field grid.', $failures);
documents_category_editor_require($template, '@media (max-width:760px)', 'Responsive single-column fallback is missing.', $failures);
documents_category_editor_require($template, 'class="documents-form-row documents-form-row--important documents-form-row--full"', 'SEO meta description is not presented as a distinct full-width block.', $failures);
...
$helpVariables = array(
    'category_help', 'cat_url_help', 'metadescription_help', 'cat_help_explanation',
    'template_help', 'css_help', 'custom_header_help', 'custom_footer_help',
    'cat_order_help', 'list_index_help', 'submitable_help', 'owner_help',
    'group_help', 'permissions_editor_help', 'action_help'
);
...
documents_category_editor_require($adminStyles, '/admin/plugins/documents/documents.css', 'Admin stylesheet helper no longer registers the Documents stylesheet.', $failures);
